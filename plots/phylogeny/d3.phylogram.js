...
function build_tree(selector, root, options) {  // , taxaToObj, maxCount) {
    ...
    var event_location = options.width - (offset - 50);
    var barWidth = 25
    var editBarWidth = 250
    var barSpacer = 10
    ...
    var maxProp = d3.max(nodes, function(x) {
        if (!x.children) {
	    return Math.max(absMax * 100.0, (+x.max_organ_prop + 0.05 * 100.0));
        } else {
	    return 0.0
        }
    })
    ...
    */
	...
	var rectangle = vis.append("rect")
	    ...
	    .attr('id', d.event )
            ...
            .attr("width", barHeight)
            .attr("height", function(d) {return barLength})
            .attr("transform","translate(" + event_location + "," + (d.x + barHeight/2.0) + ") rotate(-90) ")
	    .style('pointer-events','all')
	...
	for (i = 0; i < eventArray.length; i++) {
		...
		.attr('id', d.event )
                ...
                .attr('stroke-width', edit_stroke_width)
                .attr("width", barHeight)
            //.attr("height", function(d) {return scaleX(Math.min(scaleX(end),Math.max(scaleX(start),scaleX(eventArray[i].len))))})
		.attr("height", function(d) {return Math.min(barLength - scaleX(eventArray[i].pos),scaleX(eventArray[i].pos + eventArray[i].len) - scaleX(eventArray[i].pos))})
...
function hmidToEvents(eventString) {
    var res = Array.from(new Set(eventString.split("_")))
    var tokens = [];
...
// convert a single event string to an object
